...
class ReportesAdminController extends Controller
{
  ...
  public function balanceIngresosEgresosProcesar(BalanceGenerarRequest $request)
  {
    ...
    // Generar el PDF
    if ($tipo_reporte == 'pdf') {
      $view = \View::make('admin.reportes.balance_rept',
                          compact('fecha', 'balance', 'balance_detallado', 'nombre_tesorera')
                         )->render();
  ...
  public function ingresosPorCajera()
  {
    ...
    return view('admin.reportes.ingresos_cajera', ['cajeras' => $cajeras]);
  }
  /*
   * Mostrar la pantalla de Lista de Ingresos por Cajera
   */
  public function procesarIngresosPorCajera(Request $request)
  {
    ...
    // Generar el PDF
    if ($tipo_reporte == 'pdf') {
      $view = \View::make('admin.reportes.ingresos_cajera_rept',
                          ['ingresos' => $ingresos, 'fecha' => $fecha, 'total' => $total, 'cajera' => $nombre_cajera]
                         )->render();
      $pdf = \App::make('dompdf.wrapper');
      $pdf->loadHTML($view);
      return $pdf->stream($archivo);
  ...
  public function periodosAlumno($nro_documento)
  {
    ...
    if ($alumno) {
      ...
      $respuesta['mensaje'] = 'Alumno no se encuentra registrado.';
    }
    return $respuesta;
  }
  /*
   * Generar el Reporte de Cuenta de Alumno
   */
